...
import { readOperationalCommunicationPolicy } from "@/lib/server/operations/operational-communication-center";
...
import {
  ...
  operationalCommunicationRuleForStage,
...
async function readRecipients(db: Firestore, restaurantId: string): Promise<NotificationRecipient[]> {
  ...
  for (const document of snapshot.docs) {
    ...
    try {
      ...
      recipients.push({ uid: profile.uid, email: profile.email, role: String(profile.role ?? "") });
    } catch {
      // Profiles requiring review must never receive operational data.
    }
  }
  return recipients;
}
...
function emailHtml(copy: { title: string; body: string }, url: string): string {
  const escape = (value: string) => value
    ...
    .replaceAll('"', "&quot;")
    .replaceAll("'", "&#039;");
  return `<div style="font-family:Arial,sans-serif;color:#0f172a;line-height:1.5"><h2>${escape(copy.title)}</h2><p>${escape(copy.body)}</p><p><a href="${escape(url)}">Abrir Centro de operaciones</a></p><p style="color:#64748b;font-size:12px">Hostly · comunicación operativa automática</p></div>`;
}
...
async function sendPush(input: {
  ...
  for (let offset = 0; offset < subscriptions.length; offset += 500) {
    ...
    const result = await messaging.sendEachForMulticast({
      ...
      webpush: { headers: { Urgency: stage === "attention" ? "normal" : "high" } },
    });
...
async function dispatchAlert(input: {
  ...
  communicationPolicy: Awaited<ReturnType<typeof readOperationalCommunicationPolicy>>;
  nowMs: number;
}): Promise<{ sent: number; failed: number; skipped: number }> {
  const stage = resolveOperationalNotificationStage(input.alert);
  const rule = operationalCommunicationRuleForStage(input.communicationPolicy, stage);
  const recipients = input.recipients.filter((recipient) =>
    recipientMatchesOperationalCommunicationAudience(recipient.role, rule.audience),
  );
  let sent = 0;
  let failed = 0;
  let skipped = 0;
  for (const recipient of recipients) {
    for (const channel of ["push", "email"] as const) {
      const enabled = operationalCommunicationChannelEnabled({
        policy: input.communicationPolicy,
        stage,
        channel,
        globalChannelEnabled: channel === "push" ? input.globalPushEnabled : input.globalEmailEnabled,
        providerAvailable: channel === "push" ? input.availability.push : input.availability.email,
      });
      if (!enabled) {
        skipped += 1;
        continue;
      ...
      const result = channel === "email"
        ? await sendEmail({ recipient, alert: input.alert, idempotencyKey: `hostly-alert/${claim.id}` })
        : await sendPush({ db: input.db, restaurantId: input.restaurantId, recipient, alert: input.alert });
...
export async function dispatchOperationalCommunications(
  db: Firestore,
  nowMs = Date.now(),
): Promise<{ acquired: boolean; restaurants: number; alerts: number; sent: number; failed: number; skipped: number }> {
  ...
  try {
    ...
    for (const restaurantId of restaurantIds) {
      const [center, communicationPolicy] = await Promise.all([
        buildAndSyncOperationalAlertCenter(db, restaurantId, nowMs),
        readOperationalCommunicationPolicy(db, restaurantId),
      ]);
      if (!center.policy.enabled || !communicationPolicy.enabled || center.alerts.length === 0) continue;
      if (!center.policy.notificationChannels.push && !center.policy.notificationChannels.email) continue;
      const recipients = await readRecipients(db, restaurantId);
      ...
      for (const alert of center.alerts) {
        const result = await dispatchAlert({
          ...
          recipients,
          availability,
          globalPushEnabled: center.policy.notificationChannels.push,
          globalEmailEnabled: center.policy.notificationChannels.email,
          communicationPolicy,
          nowMs,
        });
        sent += result.sent;
