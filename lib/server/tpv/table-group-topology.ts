...
/**
 * Adapta el contrato actual (`mainTableId` + `memberTableIds`) a topología autoritativa.
 * Absorbe grupos completos de cada hint (p.ej. unir C absorbe C+D).
 * El cliente puede omitir miembros del grupo secundario; el servidor los completa.
 * Rechaza hints que no queden dentro del resultado server-side.
 */
export function planMergeFromMemberHints(args: {
  currentGroups: TableGroupsMap;
  mainTableId: string;
  clientMemberIds: readonly string[];
}): MergeFromHintsOk | TopologyErr {
  const mainHint = args.mainTableId.trim();
  if (!mainHint) return { ok: false, error: "TABLE_ID_REQUIRED" };

  const clientMembers = [
    ...new Set(args.clientMemberIds.map((id) => String(id).trim()).filter(Boolean)),
  ];

  let current = args.currentGroups;
  let mainId = resolveMainTableIdFromGroups(current, mainHint);

  for (const mid of clientMembers) {
    if (!mid || mid === mainId) continue;
    const midMain = resolveMainTableIdFromGroups(current, mid);
    if (midMain === mainId) continue;
    const step = planJoinTopology({
      currentGroups: current,
      mainTableId: mainId,
      secondaryTableId: mid,
    });
    if (!step.ok) return step;
    current = step.nextGroups;
    mainId = step.mainTableId;
  }

  const memberIds = collectGroupMemberIds(current, mainId);
  if (clientMembers.length > 0) {
    const serverSet = new Set(memberIds);
    const orphan = clientMembers.filter((id) => !serverSet.has(id));
    if (orphan.length > 0) {
      return { ok: false, error: "GROUP_TOPOLOGY_MISMATCH" };
    }
  }
...
export type SplitFromRemovedOk = {
  ok: true;
  mainTableId: string;
  effectiveMainTableId: string;
  memberIdsBefore: string[];
...
/**
 * Adapta el contrato actual de split (`removedTableIds` / `newMainTableId`)
 * a topología autoritativa. El servidor calcula remaining/nextGroups.
 */
export function planSplitFromRemovedHints(args: {
  currentGroups: TableGroupsMap;
  mainTableId: string;
  removedTableIds: readonly string[];
  newMainTableId?: string;
}): SplitFromRemovedOk | TopologyErr {
  const mainHint = args.mainTableId.trim();
  if (!mainHint) return { ok: false, error: "TABLE_ID_REQUIRED" };

  const resolvedMain = resolveMainTableIdFromGroups(args.currentGroups, mainHint);
  const memberIdsBefore = collectGroupMemberIds(args.currentGroups, resolvedMain);
  if (memberIdsBefore.length < 2) {
    return { ok: false, error: "GROUP_NOT_FOUND" };
  }
  ...
  for (const rid of removed) {
    if (!memberIdsBefore.includes(rid)) {
      return { ok: false, error: "TABLE_NOT_IN_GROUP" };
    }
  }

  const separatingPrimary = removed.includes(resolvedMain);
  const remainingTableIds = memberIdsBefore.filter((id) => !removed.includes(id));
  if (separatingPrimary) {
    const newMain = args.newMainTableId?.trim() || "";
    if (!newMain) return { ok: false, error: "NEW_MAIN_TABLE_ID_REQUIRED" };
    if (!remainingTableIds.includes(newMain)) {
      return { ok: false, error: "TABLE_NOT_IN_GROUP" };
    }
  }

  const dissolveWholeGroup = remainingTableIds.length <= 1;
  const effectiveMainTableId = separatingPrimary
    ? (args.newMainTableId?.trim() || remainingTableIds[0] || resolvedMain)
    : resolvedMain;

  const nextGroups: TableGroupsMap = {};
  for (const [main, joined] of Object.entries(args.currentGroups)) {
    if (main === resolvedMain) continue;
    const filtered = joined.filter((id) => !removed.includes(String(id).trim()));
    if (filtered.length > 0) nextGroups[main] = filtered;
  }

  if (!dissolveWholeGroup) {
    const joined = remainingTableIds
      .filter((id) => id !== effectiveMainTableId)
      .sort((a, b) => a.localeCompare(b));
    if (joined.length > 0) nextGroups[effectiveMainTableId] = joined;
  }

  return {
    ok: true,
    mainTableId: resolvedMain,
    effectiveMainTableId,
    memberIdsBefore,
