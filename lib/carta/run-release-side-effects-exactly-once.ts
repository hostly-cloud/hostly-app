...
/**
 * Effectively-once stock + print + activity por liberación lógica.
 * Orden funcional (igual que Carta hoy): stock → print → activity.
...
export async function runReleaseSideEffectsExactlyOnce(
  ...
  const stockApplied = await runOne("stock", params.runStock);
  const printed = await runOne("print", params.runPrint);
  const activityLogged = await runOne("activity", params.runActivity);

  return { releaseEventId, stockApplied, printed, activityLogged };
}
