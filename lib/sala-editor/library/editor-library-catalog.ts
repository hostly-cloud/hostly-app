...
const ESTRUCTURA_LIBRARY_CATEGORIES: readonly SalaEditorLibraryCategory[] = [
  {
    ...
    items: [
      ...
      { id: "pared-horizontal", label: "Pared horizontal", status: "available", structuralKind: "wall", wallPreset: "horizontal" },
      { id: "pared-vertical", label: "Pared vertical", status: "available", structuralKind: "wall", wallPreset: "vertical" },
  ...
  { id: "barras-fijas", label: "Barras fijas", icon: "▭", upcoming: true, items: [] },
  { id: "jardineria", label: "Jardinería", icon: "🌿", upcoming: true, items: [] },
  ...
  { id: "piscinas", label: "Piscinas", icon: "💧", upcoming: true, items: [] },
  { id: "escaleras", label: "Escaleras", icon: "🪜", upcoming: true, items: [] },
] as const;
...
const TERRENO_LIBRARY_CATEGORIES: readonly SalaEditorLibraryCategory[] = [
  ...
  {
    id: "formas-superficie",
    label: "Forma",
    icon: "◯",
    items: [
      { id: "shape-rectangle", label: "Rectangular", status: "available", surfaceShape: "rectangle" },
      { id: "shape-rounded", label: "Esquinas redondeadas", status: "available", surfaceShape: "rounded" },
...
const PAISAJISMO_LIBRARY_CATEGORIES: readonly SalaEditorLibraryCategory[] = [
  {
    id: "paisajismo", label: "Ambiente", icon: "🌿", items: [
      ...LANDSCAPE_TOOLBOX_ITEMS.map((item) => ({ id: `landscape-${item.kind}`, label: item.label, status: "available" as const, landscapeKind: item.kind })),
      { id: "landscape-tree", label: "Árbol", status: "upcoming" },
      { id: "landscape-shrub", label: "Arbusto", status: "upcoming" },
      { id: "landscape-hedge", label: "Seto", status: "upcoming" },
      { id: "landscape-flowers", label: "Flores", status: "upcoming" },
      { id: "landscape-rock", label: "Roca", status: "upcoming" },
      { id: "landscape-fountain", label: "Fuente", status: "upcoming" },
    ],
  },
] as const;
...
const OPERACION_LIBRARY_CATEGORIES: readonly SalaEditorLibraryCategory[] = [
  ...
  },
  {
    id: "servicio", label: "Puntos de servicio", icon: "🛎", items: [
      ...
      { id: "office", label: "Office", status: "upcoming" },
      { id: "lavado", label: "Lavado", status: "upcoming" },
      { id: "caja", label: "Caja", status: "upcoming" },
      { id: "espera-delivery", label: "Espera Delivery", status: "upcoming" },
    ],
  },
] as const;
...
export function getSalaEditorLibraryCategories(phase: SalaEditorLibraryPhase): readonly SalaEditorLibraryCategory[] {
  return LIBRARY_BY_PHASE[phase];
}

export function countAvailableLibraryItems(category: SalaEditorLibraryCategory): number {
...
export function getDefaultExpandedLibraryCategoryId(categories: readonly SalaEditorLibraryCategory[]): string | null {
  const firstWithTools = categories.find((category) => !category.upcoming && countAvailableLibraryItems(category) > 0);
