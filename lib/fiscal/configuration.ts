...
export type FiscalReadinessCheck = {
  key: "company" | "nif" | "address" | "series" | "taxes" | "verifactu" | "authorization" | "declaration";
  ready: boolean;
  label: string;
};
...
function assertTimeZone(timezone: string): string {
  ...
  try {
    new Intl.DateTimeFormat("es-ES", { timeZone: result }).format(new Date(0));
  } catch {
    throw new Error("FISCAL_TIMEZONE_INVALID");
  }
  return result;
}
...
function normalizeSeries(input: FiscalSeries[] | undefined): FiscalSeries[] {
  ...
  const result = rows.map((row) => {
    ...
    seen.add(code);
    if (!["simplified", "complete", "rectification"].includes(row.kind)) {
      throw new Error("FISCAL_SERIES_KIND_INVALID");
    }
    if (!Number.isInteger(row.numberPadding) || row.numberPadding < 1 || row.numberPadding > 12) {
      throw new Error("FISCAL_SERIES_PADDING_INVALID");
    }
    return {
      code,
      kind: row.kind,
      resetEachYear: row.resetEachYear !== false,
      numberPadding: row.numberPadding,
      active: row.active !== false,
    };
  });
  for (const kind of ["simplified", "complete", "rectification"] as const) {
    if (!result.some((row) => row.kind === kind && row.active)) {
      throw new Error(`FISCAL_SERIES_${kind.toUpperCase()}_REQUIRED`);
    }
  }
  return result;
}
...
export function buildFiscalConfiguration(input: {
  restaurantId: string;
  value: FiscalConfigurationInput;
  existing?: FiscalConfiguration | null;
}): FiscalConfiguration {
  ...
  if (defaultVatRateBps != null && (!Number.isInteger(defaultVatRateBps) || defaultVatRateBps < 0 || defaultVatRateBps > 10_000)) {
    throw new Error("FISCAL_DEFAULT_VAT_INVALID");
  }
  ...
  return {
    ...
    restaurantId,
    taxEntityId,
    establishmentId: existingSameTaxpayer
      ? input.existing!.establishmentId
      : stableId("est", `${restaurantId}\u0000${nif}`),
    mode,
    status: "draft",
    taxpayer: {
      id: taxEntityId,
      legalName: cleanText(input.value.taxpayerLegalName, "FISCAL_LEGAL_NAME", 120),
      nif,
      address: normalizeAddress(input.value.taxpayerAddress, "FISCAL_TAXPAYER_ADDRESS"),
    },
    ...
    software: {
      ...
      systemName: "Hostly",
      systemId: "H1",
      version: HOSTLY_FISCAL_MODULE_VERSION,
      installationNumber: existingSameTaxpayer
        ? input.existing!.software.installationNumber
        : randomUUID(),
    ...
    certificateSecretResource: existingSameTaxpayer
      ? input.existing!.certificateSecretResource
      : null,
    representationVerifiedAt: existingSameTaxpayer
      ? input.existing!.representationVerifiedAt
      : null,
    activatedAt: null,
    activatedBy: null,
  };
...
export function fiscalReadiness(config: FiscalConfiguration): FiscalReadinessCheck[] {
  const validProducer = config.software.producerNif !== "PENDING" && (() => {
    try { assertValidSpanishTaxId(config.software.producerNif); return true; } catch { return false; }
  })();
  return [
...
export function assertFiscalConfigurationCanActivate(
  ...
  if (config.software.version !== HOSTLY_FISCAL_MODULE_VERSION || HOSTLY_SIF_VERSION !== "1.0") {
    throw new Error("FISCAL_SOFTWARE_VERSION_MISMATCH");
  }
}
...
export function isStoredFiscalConfiguration(value: unknown): value is FiscalConfiguration {
  ...
  return row.schemaVersion === 1
    && typeof row.restaurantId === "string"
    && typeof row.taxEntityId === "string"
    && typeof row.establishmentId === "string"
    && (["demo", "test", "live"] as const).includes(row.mode as FiscalOperatingMode)
    && Array.isArray(row.series)
    && Boolean(row.taxpayer && row.software);
}
