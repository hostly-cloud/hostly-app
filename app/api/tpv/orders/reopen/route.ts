...
import { handleReopenTpvOrder } from "@/lib/server/tpv/handle-tpv-order-mutations";
import { parseReopenTpvOrderBody } from "@/lib/server/tpv/tpv-mutation-dtos";
import {
...
export async function POST(req: Request) {
  ...
  const parsed = parseReopenTpvOrderBody(body);
  if ("error" in parsed) {
    return NextResponse.json({ ok: false, error: parsed.error }, { status: 400 });
  }

  const result = await handleReopenTpvOrder(authCtx, parsed);
