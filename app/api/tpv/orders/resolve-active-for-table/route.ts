...
import { handleResolveActiveOrderForTable } from "@/lib/server/tpv/handle-tpv-order-mutations";
import { parseResolveActiveOrderForTableBody } from "@/lib/server/tpv/tpv-mutation-dtos";
import {
...
export async function POST(req: Request) {
  ...
  const parsed = parseResolveActiveOrderForTableBody(body);
  if ("error" in parsed) {
    return NextResponse.json({ ok: false, error: parsed.error }, { status: 400 });
  }

  const result = await handleResolveActiveOrderForTable(authCtx, parsed);
