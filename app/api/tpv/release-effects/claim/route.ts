...
export async function POST(req: Request) {
  ...
  if (!isReleaseSideEffect(effectRaw)) {
    return NextResponse.json({ ok: false, error: "RELEASE_EFFECT_INVALID" }, { status: 400 });
  }
  const leaseOwner =
    typeof body.leaseOwner === "string" ? body.leaseOwner.trim() : "";

  const result = await handleAcquireReleaseEffectLease(authCtx, {
    releaseEventId,
    effect: effectRaw,
    leaseOwner,
  });
