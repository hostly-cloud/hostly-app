...
const PRIMARY: Action[] = [
  { href: "/dashboard/operacion/tpv", label: "TPV", kind: "tpv", visible: (can) => can("tpv.sell") },
];
const OPERATION: Action[] = [
  { href: "/dashboard/operacion/cocina", label: "Cocina", kind: "kitchen", visible: (can) => can("kds.manage") },
  { href: "/dashboard/operacion/barra", label: "Barra", kind: "bar", visible: (can) => can("kds.manage") },
  { href: "/dashboard/operacion/cocteleria", label: "Coctelería", kind: "cocktail", visible: (can) => can("kds.manage") },
  { href: "/dashboard/operacion/reservas", label: "Reservas", kind: "reservations", visible: (can) => can("reservations.manage") },
];
...
const MANAGEMENT = [
  { path: "/dashboard/configuracion/carta/productos", label: "Productos", subtitle: "Carta y catálogo", kind: "products" as const, capability: "catalog.manage" as HostlyCapability },
          ...
          <header className="hostly-dashboard-command-header">
            <div className="hostly-dashboard-command-brand">
              ...
              <div className="min-w-0">
                ...
                <h1 className="hostly-dashboard-command-title">Todo listo para empezar el servicio.</h1>
              </div>
            </div>
          </header>
                  ...
                  {operation.map((a) => (
                    <Link key={a.href} href={a.href} className="hostly-dashboard-command-station" data-visual={a.kind}>
                      ...
                      <span className="hostly-dashboard-command-station__copy">
                        <span className="hostly-dashboard-command-station__label">{a.label}</span>
                        <span className="hostly-dashboard-command-station__sub">{SUBTITLES[a.label]}</span>
                      </span>
                    </Link>
                  ))}
                </nav>
                ...
                {management.map((m) => (
                  <Link key={m.path} href={m.path} className="hostly-dashboard-command-dock-item" data-visual={m.kind}>
                    ...
                    <span className="hostly-dashboard-command-dock-item__copy">
                      <span className="hostly-dashboard-command-dock-item__label">{m.label}</span>
                      <span className="hostly-dashboard-command-dock-item__sub">{m.subtitle}</span>
                    </span>
