  ...
  return (
    <LauncherIcon>
      <svg viewBox="0 0 24 24" fill="none" aria-hidden>
        <g stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">{paths[kind]}</g>
      </svg>
    </LauncherIcon>
  );
}
...
function SmallIcon({ kind, size = 22 }: { kind: "products" | "settings" | "analytics"; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
const MANAGEMENT = [
  { path: "/dashboard/configuracion/carta/productos", label: "Productos", subtitle: "Carta y catálogo", kind: "products" as const, capability: "settings.manage" as HostlyCapability },
  { path: "/dashboard/clientes", label: "Clientes", subtitle: "CRM, visitas y preferencias", kind: "analytics" as const, capability: "analytics.view" as HostlyCapability },
  { path: "/dashboard/configuracion", label: "Configuración", subtitle: "Restaurante y equipo", kind: "settings" as const, capability: "settings.manage" as HostlyCapability },
  { path: "/dashboard/analisis", label: "Análisis", subtitle: "Ventas y rendimiento", kind: "analytics" as const, capability: "analytics.view" as HostlyCapability },
];
...
export default function DashboardPage() {
  ...
  return (
    <ModulePageShell title={null} maxWidth={1280} compactLayout operationalFocus lockViewport hideBackLink shellSurface="configLight">
        ...
        <div className="hostly-dashboard-command-center">
          <header className="hostly-dashboard-command-header">
            <div className="hostly-dashboard-command-brand">
              <HostlyBrandMark className="hostly-dashboard-brand-mark" size={30} tone="app" />
              <div className="min-w-0"><p className="hostly-dashboard-command-eyebrow">{title}</p><h1 className="hostly-dashboard-command-title">Todo listo para empezar el servicio.</h1></div>
            </div>
          </header>
          <section className="hostly-dashboard-command-main" aria-label="Acciones operativas">
            {primary.length > 0 && (
              <nav aria-label="Acción principal" className="hostly-dashboard-command-hero-wrap">
                {primary.map((a) => (
                  <Link key={a.href} href={a.href} className="hostly-dashboard-command-hero">
                    <span className="hostly-dashboard-command-hero__art"><DashboardModuleArtwork kind="tpv" /></span>
                    <span className="hostly-dashboard-command-hero__icon"><StationIcon kind={a.kind} /></span>
                    <span className="hostly-dashboard-command-hero__copy"><span className="hostly-dashboard-command-hero__label">Abrir TPV</span><span className="hostly-dashboard-command-hero__sub">Mesas, pedidos y cobro</span></span>
                  </Link>
                ))}
              </nav>
            )}
            {operation.length > 0 && (
              <section aria-label="Operación" className="hostly-dashboard-command-operation">
                ...
                <nav className="hostly-dashboard-command-stations">
                  {operation.map((a) => (
                    <Link key={a.href} href={a.href} className="hostly-dashboard-command-station" data-visual={a.kind}>
                      <span className="hostly-dashboard-command-station__art"><DashboardModuleArtwork kind={a.kind as DashboardArtworkKind} /></span>
                      <span className="hostly-dashboard-command-station__icon"><StationIcon kind={a.kind} /></span>
                      <span className="hostly-dashboard-command-station__copy"><span className="hostly-dashboard-command-station__label">{a.label}</span><span className="hostly-dashboard-command-station__sub">{SUBTITLES[a.label]}</span></span>
                    </Link>
                  ))}
                </nav>
              </section>
          ...
          {management.length > 0 && (
            <section aria-label="Gestión" className="hostly-dashboard-command-management">
              ...
              <div className="hostly-dashboard-command-dock">
                {management.map((m) => (
                  <Link key={m.path} href={m.path} className="hostly-dashboard-command-dock-item" data-visual={m.kind}>
                    <span className="hostly-dashboard-command-dock-item__art"><DashboardModuleArtwork kind={m.kind} /></span>
                    <span className="hostly-dashboard-command-dock-item__icon"><SmallIcon kind={m.kind} size={18} /></span>
                    <span className="hostly-dashboard-command-dock-item__copy"><span className="hostly-dashboard-command-dock-item__label">{m.label}</span><span className="hostly-dashboard-command-dock-item__sub">{m.subtitle}</span></span>
                    <span className="hostly-dashboard-command-dock-item__arrow" aria-hidden>→</span>
                  </Link>
                ))}
              </div>
            </section>
