  ...
  return (
    <ModulePageShell title="Facturación fiscal" backHref="/dashboard" backLabel="Volver al dashboard" maxWidth={1200}>
      <div className="space-y-5">
        ...
        <HostlySurface variant="flat" className="p-4">
          <div className="flex flex-wrap gap-3"><HostlyInput className="min-w-64 flex-1" placeholder="Buscar número, cliente o NIF" value={query} onChange={(event) => setQuery(event.target.value)} /><HostlySelect value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Todos los estados</option>{Object.entries(STATUS_LABEL).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</HostlySelect><HostlyButton variant="secondary" onClick={() => void download("/api/fiscal/invoices/export", "hostly-facturacion.csv")}>Exportar CSV</HostlyButton><HostlyButton variant="secondary" onClick={() => void download("/api/fiscal/records/export", "hostly-registros-rrsif.ndjson")}>Exportar registros RRSIF</HostlyButton></div>
          <p className="mt-3 text-xs text-slate-500">El CSV es para gestión. “Registros RRSIF” descarga la copia técnica fidedigna de los registros fiscales conservados por Hostly, con manifiesto y huellas SHA-256.</p>
        </HostlySurface>
