...
const PROJECT_ID = "demo-hostly-tpv-mutations-prefix-ch";
const ADMIN_APP_NAME = "tpv-cross-handler-prefix-diagnostic";
const RESTAURANT_A = "rest-a-tpv";
const RESTAURANT_B = "rest-b-tpv-price";
const MANAGER_UID = "manager-tpv-a";
const PREFIX_IDEM_SCOPE = "cross-handler-prefix:";

function prefixIdempotencyKey(key: string): string {
  return `${PREFIX_IDEM_SCOPE}${key}`;
}
...
async function clearRestaurantTpvIdempotency(
  adminDb: Firestore,
  restaurantId: string,
): Promise<void> {
...
test("cross-handler prefix 1-37 then assign A1-A14", async () => {
  ...
  const logDiagnostic = (partial: Record<string, unknown>) => {
    console.log(
      `TPV_CROSS_HANDLER_PREFIX_DIAGNOSTIC ${JSON.stringify({
        ...partial,
        timeline,
        steps,
  ...
  try {
    ...
    await Promise.all([
      clearRestaurantTpvIdempotency(adminDb, RESTAURANT_A),
      clearRestaurantTpvIdempotency(adminDb, RESTAURANT_B),
    ]);
    timeline.push("idempotency-cache-cleared");
    ...
    const t1Created = await handleCreateOpenOrder(authCtx("waiter"), {
      ...
      markSent: true,
      idempotencyKey: prefixIdempotencyKey("create-1"),
    });
    ...
    if ("orderId" in t1Created) {
      ...
      const t1Charged = await handleChargeOrder(authCtx("waiter"), {
        ...
        type: "table_amount",
        amount: 4.5,
        idempotencyKey: prefixIdempotencyKey("charge-1"),
      });
    ...
    const t5Intent = {
      ...
      idempotencyKey: prefixIdempotencyKey("idem-create-2"),
    };
    ...
    const t6Intent = {
      ...
      idempotencyKey: prefixIdempotencyKey("merge-1"),
    };
    ...
    const t7Intent = {
      ...
      expectedStatus: "sent",
      nextStatus: "prepared",
      idempotencyKey: prefixIdempotencyKey("split-qty-1"),
    };
    ...
    const t12Created = await handleCreateOpenOrder(authCtx("waiter"), {
      ...
      markSent: true,
      idempotencyKey: prefixIdempotencyKey("create-mod-canonical"),
    });
    ...
    const t12ShadowDenied = await handleCreateOpenOrder(authCtx("waiter"), {
      ...
      markSent: true,
      idempotencyKey: prefixIdempotencyKey("create-mod-shadow-deny"),
    });
    ...
    for (const [restaurantId, delta, tableId, productId, lineId, key] of [
      ...
      const created = await handleCreateOpenOrder(
        { ...authCtx("waiter"), restaurantId },
        {
          ...
          markSent: true,
          idempotencyKey: prefixIdempotencyKey(key),
        },
      );
    ...
    const t14Created = await handleCreateOpenOrder(authCtx("waiter"), {
      ...
      markSent: true,
      idempotencyKey: prefixIdempotencyKey("create-empty-ref"),
    });
    ...
    const t15Created = await handleCreateOpenOrder(authCtx("waiter"), {
      ...
      markSent: true,
      idempotencyKey: prefixIdempotencyKey("create-inv-snap"),
    });
    ...
    const t17Created = await handleCreateOpenOrder(authCtx("waiter"), {
      ...
      markSent: true,
      idempotencyKey: prefixIdempotencyKey("create-6c2-stock"),
    });
    ...
    timeline.push("t18-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("create-6c22-bad-stock");
    ...
    timeline.push("t19-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("create-6c22-bad-unit");
      await adminDb.collection("tables").doc(tableId).set({
        restaurantId: RESTAURANT_A,
        name: "Mesa bad unit",
      });
    ...
    timeline.push("t20-start");
    {
      ...
      const created = await handleCreateOpenOrder(authCtx("waiter"), {
        ...
        markSent: true,
        idempotencyKey: prefixIdempotencyKey("create-6c22-zero-stock"),
      });
      assert.equal("orderId" in created, true);
      if ("orderId" in created) {
        const movementsSnap = await adminDb
    ...
    timeline.push("t21-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("create-6c23-invalid-stock");
    ...
    timeline.push("t22-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("upsert-6c23-invalid-stock");
      await seedInvalidCurrentStockModifierFixture(adminDb, fixture);
      const created = await handleCreateOpenOrder(authCtx("waiter"), {
        tableId: fixture.tableId,
        lines: [
        ...
        markSent: false,
        idempotencyKey: prefixIdempotencyKey("create-pending-6c23-upsert"),
      });
      assert.equal("orderId" in created, true);
      if ("orderId" in created) {
        const upsertIntent = {
    ...
    timeline.push("t23-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("status-6c23-invalid-stock");
      await seedInvalidCurrentStockModifierFixture(adminDb, fixture);
      const created = await handleCreateOpenOrder(authCtx("waiter"), {
        tableId: fixture.tableId,
        lines: [
        ...
        markSent: false,
        idempotencyKey: prefixIdempotencyKey("create-pending-6c23-status"),
      });
      assert.equal("orderId" in created, true);
      if ("orderId" in created) {
        const intent = {
    ...
    timeline.push("t24-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("qty-6c23-invalid-stock");
      await seedInvalidCurrentStockModifierFixture(adminDb, fixture);
      const created = await handleCreateOpenOrder(authCtx("waiter"), {
        tableId: fixture.tableId,
        lines: [
        ...
        markSent: false,
        idempotencyKey: prefixIdempotencyKey("create-pending-6c23-qty"),
      });
      assert.equal("orderId" in created, true);
      if ("orderId" in created) {
        const intent = {
    ...
    timeline.push("t25-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("create-6c23-catalog-fix");
      await seedInvalidCurrentStockModifierFixture(adminDb, fixture);
      const intent = {
        tableId: fixture.tableId,
        lines: [
    ...
    timeline.push("t26-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("create-6c23-empty-warnings");
      await adminDb.collection("tables").doc(tableId).set({
        restaurantId: RESTAURANT_A,
    ...
    timeline.push("t27-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("create-6c23-multi-warnings");
      await adminDb.collection("tables").doc(tableId).set({
        restaurantId: RESTAURANT_A,
    ...
    timeline.push("t28-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("create-6c23-conflict");
    ...
    timeline.push("t29-start");
    {
      ...
      const idempotencyKey = prefixIdempotencyKey("create-6c23-legacy");
      await adminDb.collection("tables").doc(tableId).set({
        restaurantId: RESTAURANT_A,
        name: "Mesa legacy",
      });
    ...
    const t37Created = await handleCreateOpenOrder(authCtx("waiter"), {
      ...
      idempotencyKey: prefixIdempotencyKey("create-op-assign-1"),
    });
